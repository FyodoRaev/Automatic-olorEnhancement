...
#define CHECK_DIM(x, n) TORCH_CHECK(x.ndimension() == n, #x " must be " #n "D")
                        ...
                        bool has_offset)
{
        ...
        TORCH_CHECK((input.sizes()[0] == guide.sizes()[0]) &&
                 (input.sizes()[2] == h) &&
                 (input.sizes()[3] == w),
                 "Input and guide size should match");
        TORCH_CHECK(guide.sizes()[0] == bs, "Batch sizes should match.");

        int64_t output_chans;
        if (has_offset) {
                TORCH_CHECK((coeffs_chans % (input_chans + 1)) == 0,
                         "Slicing with affine offset, coefficients grid "
                         "should have n_out*(n_in+1) channels.");
                output_chans = coeffs_chans/(input_chans + 1);
        } else {
                TORCH_CHECK((coeffs_chans % input_chans) == 0,
